...
// netlify/functions/db-attendance-check.js
var db_attendance_check_exports = {};
__export(db_attendance_check_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(db_attendance_check_exports);
  ...
  try {
    ...
    try {
      ...
      if (action === "check-out" && hasOpenAttendance) {
        ...
        const personnelQuery2 = await client.query(
          "SELECT hourly_wage, daily_wage FROM personnel WHERE id = $1",
          [personnelId]
        );
        const hourlyWage = personnelQuery2.rows[0]?.hourly_wage || 0;
...
//# sourceMappingURL=db-attendance-check.js.map
