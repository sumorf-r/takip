...
// netlify/functions/payroll-calculate.js
...
function calculatePayroll(data, taxRates) {
  ...
  const hourlyWage = baseSalary / 26 / 8;
  ...
  const stampTax = grossSalary * (taxRates.stamp_tax || 759e-5);
  const totalDeductions = sskEmployee + unemploymentInsurance + incomeTax + stampTax + (advanceDeduction || 0) + (penaltyDeduction || 0);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const totalWorkDays = periodEnd.getDate() - 4;
...
//# sourceMappingURL=payroll-calculate.js.map
